...
FlowView::FlowView(FlowScene* scene, QWidget* parent)
	...
	connect(_centerOnAnimation, &QVariantAnimation::valueChanged, this, [this](QVariant const& value) {
...
bool FlowView::handleMouseReleaseEvent(QMouseEvent* event) {
	...
	auto const valid = std::all_of(std::begin(descriptors), std::end(descriptors), [this](FlowConnectionDescriptor const& descriptor) {
	...
	if (valid) {
		...
		if (maybeNodeUid) {
			...
			connect(animation, &QVariantAnimation::valueChanged, this, [this, uid = *maybeNodeUid](QVariant const& value) {
...
bool FlowView::canConnect(FlowSocket* socket) const {
	switch (_mode) {
		...
		case ConnectionMode::ChangeInput:
			if (socket->type() == FlowInput::Type) {
				auto* input = qgraphicsitem_cast<FlowInput*>(socket);
				return std::all_of(std::begin(_connections), std::end(_connections), [this, input](FlowConnection* connection) {
		...
		case ConnectionMode::ChangeOutput:
			if (socket->type() == FlowOutput::Type) {
				auto* output = qgraphicsitem_cast<FlowOutput*>(socket);
				return std::all_of(std::begin(_connections), std::end(_connections), [this, output](FlowConnection* connection) {
